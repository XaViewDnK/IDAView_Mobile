...
def export_idaview():
    ...
    for seg_ea in idautils.Segments():
        ...
        ea = seg.start_ea
        while ea < seg.end_ea:
            chunk_size = min(16, seg.end_ea - ea)
            b = ida_bytes.get_bytes(ea, chunk_size)
            if b:
                hex_str = " ".join(f"{x:02X}" for x in b)
                ascii_str = "".join(chr(x) if 32 <= x <= 126 else "." for x in b)
            else:
                hex_str = " ".join("??" for _ in range(chunk_size))
                ascii_str = "." * chunk_size
            hex_lines.append(f"{ea:08X}\t{hex_str}\t{ascii_str}")
            ea += chunk_size

    print("[*] Собираем Item Hex (для Visual: Show HEX)...")
    item_hex_lines = []
    for head in idautils.Heads(min_ea, max_ea):
        size = ida_bytes.get_item_size(head)
        if size > 0:
            b = ida_bytes.get_bytes(head, size)
            if b:
                hex_str = " ".join(f"{x:02X}" for x in b)
                item_hex_lines.append(f"{head:08X}\t{hex_str}")
    ...
    with zipfile.ZipFile(out_zip, 'w', zipfile.ZIP_DEFLATED, compresslevel=1) as zf:
        ...
        zf.writestr("hex_view.txt", "\n".join(hex_lines))
        zf.writestr("item_hex.txt", "\n".join(item_hex_lines))
